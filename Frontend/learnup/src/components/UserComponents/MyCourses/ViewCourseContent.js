...
const ViewCourseContent = () => {
  ...
  const handleTranscript = async()=>{
    try {
      ...
      const yourPathValue = `/home/armin/Downloads/FYP2_Demo/media/uploads/` + videoPath;

      console.log(yourPathValue);

      const response = await axios.post('https://785a-154-81-230-197.ngrok-free.app/transcribe',
  ...
  const handleGenerateQuiz = async () => {
    try {
      const response = await axios.post(quiz_url,
  ...
  return (
    <>
      ...
      <div className='generatequizcontainer'>
        <button className='generatequiz' onClick={handleGenerateQuiz}>Generate Quiz</button>
      </div>
      {showFeedbackPopup && (
        <div className="feedback-popup">
          <h3 className='feedbackheadingcourse'>Course Completed!</h3>
      ...
      <div className='transcriptesgenerated'>
  <h3 className='transcriptsgeneratedheading'>Transcript</h3>
  <p className='transcriptsgeneratedparagraph' style={{ whiteSpace: 'pre-line' }}>{transcript}</p>
  <div className='transcript-download' onClick={handleDownloadTranscript}>
    <span className='transcript-download-icon'>
    <MdFileDownload />
  ...
  <div className='quizzes'>
          <h3>Quizzes</h3>
          <p className='quiz_generated' style={{ whiteSpace: 'pre-line' }}>{quizzes}</p>
        </div>
    </>
  );
};
