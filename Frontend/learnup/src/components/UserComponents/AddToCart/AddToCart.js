...
const AddToCart = () => {
  ...
  const makePayment = async () => {
    try {
      ...
      const stripe = await loadStripe("pk_test_51N5ODiJmX5zAkZ90Zu2KPerrFEoP7dBF0bCJQXuLqpXv2vsCu7ALW8SmVIkgkz6oflsIyRQciWgEhO5aJGN1pePc00awoXfulJ");
      const response = await axios.get(`/api/v1/carts/${userEmail}/cart`);
  ...
  <div className="cart-item-container">
      ...
      {cart.items.map((item) => (
        <li key={item.course._id} className="cart-item">
          ...
          <div className="cart-item-details">
            ...
            <p className="cart-item-price">Price: RS{item.course.courseprice.toFixed(2)}</p>
    ...
    <div className="total-payment-container">
      <p className="total-payment">Total Payment: RS{calculateTotalPayment().toFixed(2)}</p>
