...
const Courselist = () => {
  ...
  <div className='courselistheader'>
    ...
    <div className='courselistcontentright'>
        <NavLink to="/admin-dashboard/add-course"><FaPlus className='iconpluscourselist'/><span>Add New Course</span></NavLink>
    </div>
  </div>
  <div className='courselistbar'>
    <input type='text' placeholder='Enter Course name' value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)} className='courselistsearchbar' />
        ...
        <table className='tabledatacourselist'>
          ...
          {filteredCourses.map((course, index) => (
            <tr key={course._id}>
              ...
              <td>
              <span><FaEdit className='courselistedit' onClick={()=>handleupdateicon(course._id)}/></span>
              <span>   <FaTrash className='courselisttrash' onClick={() => handleDeleteCourse(course._id)}/></span>
